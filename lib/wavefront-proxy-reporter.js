 ...
 * @constructor
 */
function WavefrontProxyReporter(registry, prefix, host, port, globaltags) {
  WavefrontProxyReporter.super_.call(this, registry);
  this.prefix = prefix;
  this.host = host;
  this.port = port || 2878;
  this.globaltags = {};
...
util.inherits(WavefrontProxyReporter, ScheduledReporter);

WavefrontProxyReporter.prototype.start = function(intervalInMs) {
  var self = this;
  ...
  this.socket.connect(this.port, this.host, function() {
    ...
    WavefrontProxyReporter.super_.prototype.start.call(self, intervalInMs);
  });
};

WavefrontProxyReporter.prototype.stop = function() {
  WavefrontProxyReporter.super_.prototype.stop.call(this);
  this.socket.end();
};

WavefrontProxyReporter.prototype.report = function() {
...
WavefrontProxyReporter.prototype.send = function(name, value, timestamp, tags, prefixed) {
  if(reconnecting) {
    return;
  }
  if (!tags) { tags = ''; }
  ...
  if (prefixed) {
    console.log(util.format('%s %s %s %s', name, value, timestamp, tags));
    this.socket.write(util.format('%s %s %s %s\n', name, value, timestamp, tags));
  } else {
    console.log(util.format('%s.%s %s %s %s', this.prefix, name, value,
      timestamp, tags));
...
WavefrontProxyReporter.prototype.reportCounter = function(counter, timestamp) {
...
WavefrontProxyReporter.prototype.reportTimer = function(timer, timestamp) {
  var send = this.send.bind(this);
...
WavefrontProxyReporter.prototype.reportHistogram = function(histogram, timestamp) {
  var send = this.send.bind(this);
